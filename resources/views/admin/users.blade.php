                ...
                <div class="card-innr">
                            ...
                            <div class="toggle-class dropdown-content dropdown-content-center-left pd-2x">
                                <div class="card-opt data-action-list">
                                    <ul class="btn-grp btn-grp-block guttar-20px guttar-vr-10px">
                                        <li>
                                            <a href="#" class="btn btn-auto btn-sm btn-primary" data-toggle="modal"
                                                data-target="#addUser">
                    ...
                    <div class="page-nav-wrap">
                            ...
                            <div class="page-nav w-100 w-lg-auto">
                                <ul class="nav">
                                    <li class="nav-item{{ is_page('users.user') ? ' active' : '' }}"><a class="nav-link"
                                            href="{{ route('admin.users', 'user') }}">Users</a></li>
                                    <li class="nav-item {{ is_page('users.admin') ? ' active' : '' }}"><a
                                            class="nav-link" href="{{ route('admin.users', 'admin') }}">Admin Account</a>
                                    </li>
                                    <li class="nav-item {{ is_page('users') ? ' active' : '' }}"><a class="nav-link"
                                            href="{{ route('admin.users') }}">All</a></li>
                                </ul>
                            </div>
                            ...
                            @if (!empty(app_key()))
                                <div class="tools w-100 w-sm-auto">
                                    <ul class="btn-grp guttar-8px">
                                        ...
                                        <li><a href="javascript:void(0)" title="Delete all unverified users"
                                                data-toggle="tooltip"
                                                class="btn btn-danger btn-icon btn-outline btn-sm delete-unverified-user mr-md-2">
                                                <em class="ti ti-trash"></em> </a></li>
                                        </form>
                                        </li>
                                    ...
                                    <div class="row align-items-end guttar-20px guttar-vr-15px">
                                        ...
                                        <div class="col-lg-4 col-sm-8 col-mb-6">
                                            <div class="input-wrap input-item-middle text-left">
                                                <input {{ request()->get('adm') == 'yes' ? 'checked' : '' }} name="adm"
                                                    value="yes" class="input-checkbox input-checkbox-md" id="include-admin"
                                                    type="checkbox">
                                        ...
                                        <div class="col-sm-4 col-lg-3 col-mb-6">
                                            <div class="input-wrap input-with-label">
                                                <label class="input-item-label input-item-label-s2 text-exlight">Account
                                                    Status</label>
                                                <select name="state" class="select select-sm select-block select-bordered"
                                                    ...
                                                    <option {{ request()->get('state') == 'suspend' ? ' selected' : '' }}
                                                        value="suspend">Suspended</option>
                                                </select>
                                            </div>
                                        </div>
                        ...
                        <div class="search-adv-result">
                            <div class="search-info">Found <span class="search-count">{{ $users->total() }}</span>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="fake-class">
                                                    ...
                                                    <span class="sub user-id">{{ set_id($user->id, 'user') }}
                                                        @if ($user->role == 'admin')
                                                            <span
                                                                class="badge badge-xs badge-dim badge-{{ $user->type != 'demo' ? 'success' : 'danger' }}">ADMIN</span>
                                                        @endif
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="relative d-inline-block">
                                                ...
                                                <div class="toggle-class dropdown-content dropdown-content-top-left">
                                                    <ul class="dropdown-list more-menu-{{ $user->id }}">
                                                        <li><a
                                                                href="{{ route('admin.users.view', [$user->id, 'details']) }}"><em
                                                                    class="far fa-eye"></em> View Details</a></li>
                                                        <li><a class="user-email-action" href="#EmailUser"
                                                                data-uid="{{ $user->id }}" data-toggle="modal"><em
                                                                    class="far fa-envelope"></em>Send Email</a></li>
                                                        @if ($user->role == 'user')
                                                            <li><a href="javascript:void(0)" data-uid="{{ $user->id }}"
                                                                    data-type="transactions"
                                                                    class="user-form-action user-action"><em
                                                                        class="fas fa-random"></em>Transaction</a></li>
                                                        @endif
                                                        <li><a href="javascript:void(0)" data-uid="{{ $user->id }}"
                                                                data-type="activities"
                                                                class="user-form-action user-action"><em
                                                                    class="fas fa-sign-out-alt"></em>Activities</a></li>
                                                        <li><a href="javascript:void(0)" data-uid="{{ $user->id }}"
                                                                data-type="referrals"
                                                                class="user-form-action user-action"><em
                                                                    class="fas fa-users"></em>Referrals</a></li>

                                                        @if ($user->id != save_gmeta('site_super_admin')->value)
                                                            <li><a class="user-form-action user-action" href="#"
                                                                    data-type="reset_pwd"
                                                                    data-uid="{{ $user->id }}"><em
                                                                        class="fas fa-shield-alt"></em>Reset Pass</a></li>
                                                        @endif
                                                        @if ($user->google2fa == 1)
                                                            <li><a class="user-form-action user-action"
                                                                    href="javascript:void(0)" data-type="reset_2fa"
                                                                    data-uid="{{ $user->id }}"><em
                                                                        class="fas fa-unlink"></em>Reset 2FA</a></li>
                                                        @endif

                                                        @if (Auth::id() != $user->id && $user->id != save_gmeta('site_super_admin')->value)
                                                            @if ($user->status != 'suspend')
                                                                <li><a href="#" data-uid="{{ $user->id }}"
                                                                        data-type="suspend_user"
                                                                        class="user-action front"><em
                                                                            class="fas fa-ban"></em>Suspend</a></li>

                                                            @else
                                                                <li><a href="#" id="front" data-uid="{{ $user->id }}"
                                                                        data-type="active_user" class="user-action"><em
                                                                            class="fas fa-ban"></em>Active</a></li>
                                                            @endif
                                                        @endif
                                                    ...
                                                    -->
                                                            <a href="#" data-uid="{{ $user->id }}"
                                                                data-type="delete_user"
                                                                data-title="Are you sure you want to delete this User?"
                                                                data-subtitle="After deleting this entry, it cannot be recovered."
                                                                class="user-action front"
                                                                data-url="{{ route('admin.delete_users', encrypt($user->id)) }}">
