...
//        App::setLocale($lang);
//        Setting::updateValue(auth()->id(), $lang);
        $update_user=Setting::where('field',auth()->id())->first();
        if ($update_user) {
            $update_user->update([
                'value'=>$lang
            ]);
...
//        Cookie::queue(Cookie::unqueue('app_language'));

//            dd(current_lang());
        $_key = Cookie::queue(Cookie::make('app_language', $lang, (60 * 24 * 365)));
//        $cookie = cookie('app_language', 'value', (60 * 24 * 365));
//        dd(Lang::getLocale(),$lang,current_lang(),Cookie::get('app_language'),Cookie::get('app_language'),$cookie);
